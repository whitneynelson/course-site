...
Confirm they have, from the pre-event checklist: a GitHub account, Git installed, VS Code or Antigravity installed, and a Morehouse Supercomputing Facility account (not used yet, but should exist). If any are missing, help them get that one thing installed before moving on; don't try to debug repo setup and tool installation at the same time.
